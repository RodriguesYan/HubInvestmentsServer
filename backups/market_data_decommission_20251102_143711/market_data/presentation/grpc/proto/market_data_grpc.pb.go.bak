...
const (
	MarketDataService_GetMarketData_FullMethodName    = "/market_data.MarketDataService/GetMarketData"
	MarketDataService_StreamMarketData_FullMethodName = "/market_data.MarketDataService/StreamMarketData"
)
...
type MarketDataServiceClient interface {
	...
	StreamMarketData(ctx context.Context, in *StreamMarketDataRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[MarketDataUpdate], error)
}
...
func (c *marketDataServiceClient) StreamMarketData(ctx context.Context, in *StreamMarketDataRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[MarketDataUpdate], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	...
	x := &grpc.GenericClientStream[StreamMarketDataRequest, MarketDataUpdate]{ClientStream: stream}
...
type MarketDataService_StreamMarketDataClient = grpc.ServerStreamingClient[MarketDataUpdate]
...
type MarketDataServiceServer interface {
	...
	// StreamMarketData provides real-time market data updates (for future implementation)
	StreamMarketData(*StreamMarketDataRequest, grpc.ServerStreamingServer[MarketDataUpdate]) error
...
func (UnimplementedMarketDataServiceServer) StreamMarketData(*StreamMarketDataRequest, grpc.ServerStreamingServer[MarketDataUpdate]) error {
...
func _MarketDataService_StreamMarketData_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(StreamMarketDataRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(MarketDataServiceServer).StreamMarketData(m, &grpc.GenericServerStream[StreamMarketDataRequest, MarketDataUpdate]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type MarketDataService_StreamMarketDataServer = grpc.ServerStreamingServer[MarketDataUpdate]
